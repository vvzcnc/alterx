...
class func:
    ...
    def execute(self):
        if self.that.jog.jog_encoder:
            _logger.info( "JOG joystick mode" )
            self.that.jog.jog_encoder = 0
        else:
            _logger.info( "JOG encoder mode" )
            self.that.jog.jog_encoder = 1

        self.update_image(self.that.jog.jog_encoder)
