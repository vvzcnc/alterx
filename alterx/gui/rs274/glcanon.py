...
class GLCanon(Translated, ArcsToSegmentsMixin):
    ...
    def calc_extents(self):
        self.min_extents, self.max_extents, self.min_extents_notool, self.max_extents_notool = gcode.calc_extents(
            self.arcfeed, self.feed, self.traverse)
        if self.is_foam:
    ...
    def straight_traverse(self, x, y, z, a, b, c, u, v, w):
        ...
        if not self.first_move:
            self.traverse_append(
                (self.lineno, self.lo, l, [self.xo, self.yo, self.zo]))
        self.lo = l
...
#        self.dwells_append((self.lineno, self.colors['dwell'], x + self.offset_x, y + self.offset_y, z + self.offset_z, 0))
    ...
    def color_with_alpha(self, name):
        glColor4f(
            *(self.colors[name] + (self.colors.get(name+'_alpha', 1/3.),)))
    ...
    def draw(self, for_selection=0, no_traverse=True):
        if not no_traverse:
            ...
            self.colored_lines('straight_feed', self.feed,
                               for_selection, len(self.traverse))

            self.colored_lines('arc_feed', self.arcfeed, for_selection, len(
                self.traverse) + len(self.feed))

            glLineWidth(2)
            self.draw_dwells(self.dwells, self.colors.get('dwell_alpha', 1/3.),
                             for_selection, len(self.traverse) + len(self.feed) + len(self.arcfeed))
            glLineWidth(1)
...
class GlCanonDraw:
    ...
    def set_highlight_line(self, line):
        if line == self.get_highlight_line():
            return
    ...
    def redraw_perspective(self):
        ...
        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        gluPerspective(self.fovy, float(w)/float(h),
    ...
    def show_extents(self):
        ...
        if view != z and g.max_extents[z] > g.min_extents[z]:
            ...
            bbox = self.color_limit(
                g.min_extents_notool[z] < machine_limit_min[z])
            glPushMatrix()
            ...
            bbox = self.color_limit(
                g.max_extents_notool[z] > machine_limit_max[z])
            glPushMatrix()
        ...
        if view != y and g.max_extents[y] > g.min_extents[y]:
            ...
            bbox = self.color_limit(
                g.min_extents_notool[y] < machine_limit_min[y])
            glPushMatrix()
            ...
            bbox = self.color_limit(
                g.max_extents_notool[y] > machine_limit_max[y])
            glPushMatrix()
        ...
        if view != x and g.max_extents[x] > g.min_extents[x]:
            ...
            bbox = self.color_limit(
                g.min_extents_notool[x] < machine_limit_min[x])
            glPushMatrix()
            ...
            bbox = self.color_limit(
                g.max_extents_notool[x] > machine_limit_max[x])
            glPushMatrix()
    ...
    def get_grid(self):
        if self.canon and self.canon.grid:
            return self.canon.grid
        return 5./25.4
    ...
    def draw_grid(self):
        ...
        if rotation != 0 and view != z and self.get_show_relative():
            return
        permutations = [
    ...
    def all_joints_homed(self):
        for i in range(self.stat.joints):
            if not self.stat.homed[i]:
                return False
        return True

    def one_or_more_on_limit(self):
        for i in range(self.stat.joints):
            if self.stat.limit[i]:
                return True
        return False
    ...
    def idx_for_home_or_limit_icon(self, string):
        ...
        if (aletter in ["X", "Y", "Z", "A", "B", "C", "U", "V", "W", "Rad", "Dia"]
            ...
            if self.all_joints_homed():
                ans = ans - 2  # allhomeicon on all letters
            if self.one_or_more_on_limit():
                ans = ans - 4  # limitedicon on all letters
        if (ans < 0):
            return ans  # -2,-4,-6
    ...
    def redraw(self):
        ...
        if self.get_show_live_plot() or self.get_show_program():
            ...
            if self.is_foam():
                ...
                glCallList(alist)
                uwalist = self.dlist(
                    ('axes_uw', self.get_view()), gen=lambda n: self.draw_axes(n, 'UVW'))
                glTranslatef(0, 0, self.get_foam_w()-self.get_foam_z())
                glCallList(uwalist)
            else:
                glCallList(alist)
        ...
        if self.get_show_limits():
            glTranslatef(
                *[-x for x in self.to_internal_units(s.tool_offset)[:3]])
            glLineWidth(1)
    ...
    def posstrs(self):
        ...
        if not self.get_joints_mode() or self.no_joint_display:
            ...
            limit, homed, posstrs, droposstrs = self.dro_format(
                self.stat, spd, dtg, limit, homed, positions, axisdtg, g5x_offset, g92_offset, tlo_offset)
        else:
    ...
    def lathetool(self, current_tool):
        ...
        w = 3/8.
        # lathe tool needs to be visable form both sides
        ...
        if orientation == 9:
            ...
            for i in range(37):
                ...
                t = circleminangle + i * (circlemaxangle - circleminangle)/36.
                glVertex3f(radius*dx + radius * math.sin(t),
                           0.0, radius*dy + radius * math.cos(t))
    ...
    def extents_info(self):
        if self.canon:
            mid = [(a+b)/2 for a, b in zip(self.canon.max_extents,
                                           self.canon.min_extents)]
            size = [(a-b)
                    for a, b in zip(self.canon.max_extents, self.canon.min_extents)]
        else:
            mid = [0, 0, 0]
            size = [3, 3, 3]
        return mid, size
