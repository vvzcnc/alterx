...
class SideButton(QPushButton):
	...
	def set_active(self,state):
		...
		icon = os.path.join(IMAGE_DIR,"mainmenu","{}{}.png".format(self.label,"_on" if state==True else ""))
...
class GCodeWidget(QGroupBox):
	...
	def on_gcode_changed(self,data):
		...
		for i in sorted(data[1:]):
			if i == -1: 
				continue
			active = active + "G{:.0f}".format(i/10) + (".{} ".format(i%10) if i%10 != 0 else " ")
...
class SpindleWidget(QGroupBox):
	def __init__(self, parent=None):
		...
		v1 = QVBoxLayout()
		self.actual_spindlerate = QLabel(_("Actual: {:.2f} {}/{}",0,INFO.spindle_units,INFO.spindle_per_units))
	...
	def on_actualrate_changed(self,speed):
		self.actual_spindlerate.setText(_("Actual: {:.2f} {}/{}",speed,INFO.spindle_units,INFO.spindle_per_units))
...
class FeedWidget(QGroupBox):
	...
	def on_actualrate_changed(self,feed):
		self.actual_feedrate.setText(_("Actual: {:.2f} {}/{}",feed*60,INFO.linear_units,INFO.feed_per_units))
...
class JOGWidget(QGroupBox):
	def __init__(self, parent=None):
		...
		self.label_jog_increment = QLabel(_("Increment: {}",self.jog_increment))
		...
		v2 = QVBoxLayout()
		self.label_jog_activate = QLabel(_("JOG ")+self.activate[self.jog_activate])
		...
		self.label_jog_device = QLabel(_("Device: ")+self.device[self.jog_device])
	...
	def on_increment_changed(self, i):
		...
		self.label_jog_increment.setText(_("Increment: {}",self.jog_increment))

	def on_active_changed(self, active):
		self.jog_activate = active
		self.label_jog_activate.setText(_("JOG ")+self.activate[self.jog_activate])
	...
	def on_device_changed(self, encoder):
		...
		self.label_jog_device.setText(_("Device: ")+self.device[self.jog_device])
...
class MainLayout(QVBoxLayout):
	def __init__(self, parent=None):
		...
		self.bottomWidgets.currentChanged.connect(lambda i: UPDATER.emit("secondary_mode", i))
